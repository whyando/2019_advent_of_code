#include <bits/stdc++.h>

using namespace std;

const vector<int64_t> program = {3,1033,1008,1033,1,1032,1005,1032,31,1008,1033,2,1032,1005,1032,58,1008,1033,3,1032,1005,1032,81,1008,1033,4,1032,1005,1032,104,99,101,0,1034,1039,1002,1036,1,1041,1001,1035,-1,1040,1008,1038,0,1043,102,-1,1043,1032,1,1037,1032,1042,1105,1,124,1001,1034,0,1039,1001,1036,0,1041,1001,1035,1,1040,1008,1038,0,1043,1,1037,1038,1042,1105,1,124,1001,1034,-1,1039,1008,1036,0,1041,1001,1035,0,1040,101,0,1038,1043,102,1,1037,1042,1106,0,124,1001,1034,1,1039,1008,1036,0,1041,1001,1035,0,1040,101,0,1038,1043,1001,1037,0,1042,1006,1039,217,1006,1040,217,1008,1039,40,1032,1005,1032,217,1008,1040,40,1032,1005,1032,217,1008,1039,9,1032,1006,1032,165,1008,1040,9,1032,1006,1032,165,1101,2,0,1044,1106,0,224,2,1041,1043,1032,1006,1032,179,1102,1,1,1044,1105,1,224,1,1041,1043,1032,1006,1032,217,1,1042,1043,1032,1001,1032,-1,1032,1002,1032,39,1032,1,1032,1039,1032,101,-1,1032,1032,101,252,1032,211,1007,0,73,1044,1105,1,224,1101,0,0,1044,1106,0,224,1006,1044,247,101,0,1039,1034,102,1,1040,1035,102,1,1041,1036,1002,1043,1,1038,1001,1042,0,1037,4,1044,1106,0,0,57,60,59,78,42,22,33,69,5,77,92,10,55,22,99,62,27,32,75,13,82,48,40,83,95,38,62,65,70,77,79,61,2,47,27,84,46,48,16,15,87,87,23,95,97,16,93,79,27,7,98,97,76,44,6,75,85,51,8,91,94,99,35,28,84,67,83,82,1,80,40,99,81,92,41,97,87,28,81,52,93,37,27,85,76,4,18,80,96,61,83,16,90,86,77,8,76,55,51,61,72,90,90,5,96,75,13,56,40,82,11,97,21,55,95,17,93,97,16,91,30,77,28,32,77,96,49,13,97,30,14,26,93,61,18,32,85,95,81,65,98,49,65,84,46,19,81,45,76,22,88,79,63,84,60,24,37,4,34,80,98,61,95,46,88,99,76,3,92,75,12,95,9,98,94,57,41,77,52,17,80,83,17,83,59,87,85,2,95,88,41,32,98,72,95,23,91,83,65,82,47,90,17,81,67,81,6,90,61,44,85,40,85,19,82,26,86,1,74,62,6,75,98,93,12,84,11,38,57,88,91,76,66,43,46,84,38,48,99,57,20,97,34,72,75,47,94,33,83,82,55,76,34,90,9,74,34,87,18,84,57,38,76,3,31,87,77,2,86,15,23,92,37,20,75,94,27,88,90,87,28,84,31,92,15,75,14,10,82,48,99,49,79,4,96,91,30,16,78,7,75,67,98,1,75,49,70,87,95,89,30,54,31,92,71,57,39,79,19,55,85,25,92,4,23,77,74,77,89,40,39,77,87,39,44,81,98,80,94,29,40,88,69,74,92,54,2,1,83,30,87,85,15,80,97,94,15,93,89,71,69,86,81,75,56,92,58,95,51,93,29,82,79,24,82,9,46,5,88,53,75,90,31,6,91,69,88,8,10,23,67,5,85,78,71,36,98,19,98,53,66,43,17,91,62,28,86,44,79,33,91,79,66,71,86,98,86,60,83,44,94,96,29,85,89,50,85,98,14,8,43,5,75,62,89,90,33,30,86,48,75,99,88,82,45,99,38,60,76,69,45,89,27,93,71,84,96,15,52,85,58,97,99,20,5,79,75,33,80,84,63,87,1,77,31,44,49,99,79,3,83,16,91,42,7,99,66,82,93,76,31,55,77,30,45,88,3,81,53,30,95,24,90,96,26,33,93,32,40,25,76,21,75,17,89,31,81,18,29,97,72,60,90,14,99,93,9,55,92,26,86,28,99,93,1,98,86,60,26,68,95,7,17,86,94,48,68,95,94,88,86,67,82,77,55,76,68,79,76,65,76,21,23,78,28,82,75,23,84,74,67,69,83,70,93,38,75,81,61,87,23,86,92,43,13,23,81,65,65,76,41,34,95,64,89,90,72,79,16,47,84,47,28,83,74,1,41,59,87,84,34,32,94,4,78,37,17,99,86,91,58,97,69,26,93,41,81,36,78,80,18,79,96,51,37,74,45,94,54,98,90,74,27,8,89,79,81,71,60,88,40,86,33,95,98,21,13,9,20,94,48,82,42,69,15,84,45,89,42,98,99,72,72,84,86,54,80,74,68,74,5,85,80,75,34,29,98,16,85,16,86,13,25,74,95,51,86,90,28,81,90,20,70,7,89,37,74,28,86,29,81,95,66,1,64,83,85,27,74,4,69,54,79,66,50,96,43,94,95,45,52,83,17,37,88,85,55,35,66,78,66,86,4,92,2,99,35,89,13,76,71,81,92,96,18,85,68,95,61,97,76,82,66,85,99,82,6,93,31,81,76,80,27,95,38,94,85,98,41,91,0,0,21,21,1,10,1,0,0,0,0,0,0};

class Computer {
public:
    Computer(const vector<int64_t> program)
        :v(program)
    {}

    queue<int64_t> in;
    queue<int64_t> out;
    bool halted = false;

    // returns true if the computer actually did anything this step
    // only time this is false if it is waiting for an input
    // or it halts
    bool step(){
        int64_t opcode = v[ip] % 100;
        // parameter modes
        int64_t pm1 = (v[ip]/100)%10;
        int64_t pm2 = (v[ip]/1000)%10;
        int64_t pm3 = (v[ip]/10000)%10;
        // parameters
        int64_t *p1,*p2,*p3;

        // load the correct number of parameters
        switch(opcode){
            case 1:
            case 2:
            case 7:
            case 8:
                p3 = get_param(ip+3, pm3);
            case 5:
            case 6:
                p2 = get_param(ip+2, pm2);
            case 3:
            case 4:
            case 9:
                p1 = get_param(ip+1, pm1);
            case 99:
                break;
            default:
                cout << "unknown parameter count for opcode " << opcode << endl;
                exit(-1);
        }

        if(opcode == 1) {
            assert(pm3 != 1);
            *p3 = *p1 + *p2;
            ip += 4;
        }
        else if(opcode == 2) {
            assert(pm3 != 1);
            *p3 = *p1 * *p2;
            ip += 4;
        }
        else if(opcode == 3) {
            if(in.size() == 0){
                cout << "\tWAIT" << endl;
                return false;
            }
            *p1 = in.front(); in.pop();
            ip += 2;
        } 
        else if(opcode == 4) {
            out.push(*p1);
            // cout << "output:" << *p1 << endl;
            ip += 2;
        } 
        else if(opcode == 5) {
            // cout << "jump-if-true" << endl;
            if(*p1 != 0)    ip = *p2;
            else            ip += 3;       
        } 
        else if(opcode == 6) {
            // cout << "jump-if-false" << endl;
            if(*p1 == 0)    ip = *p2;
            else            ip += 3;
        }
        else if(opcode == 7) {
            // cout << "less than" << endl;
            if(*p1 < *p2)   *p3 = 1;
            else            *p3 = 0;
            ip += 4;
        }
        else if(opcode == 8) {
            // cout << "equal" << endl;
            if(*p1 == *p2)  *p3 = 1;
            else            *p3 = 0;
            ip += 4;
        }
        else if(opcode == 9) {
            // cout << "rebase" << endl;
            relative_base += *p1;
            ip += 2;
        }
        else if(opcode == 99) {
            cout << "\tHALT" << endl;
            halted = true;
            return false; 
            ip+=1; // technically instruction point gets stuck on 99 opcode
        }
        else {
            cout << "ERROR opcode=" << v[ip] << endl;
            exit(-1);
        }
        return true;   
    }
private:
    // instruction pointer
    int64_t ip=0;
    // memory
    vector<int64_t> v;
    int64_t relative_base = 0;

    int64_t *get_param(int64_t param_ip, int64_t param_mode){
        int index;
        if(param_mode == 0) index = v[param_ip];
        if(param_mode == 1) index = param_ip;
        if(param_mode == 2) index = v[param_ip] + relative_base;
        if(index >= v.size()){
            cout << "Expanding to size " << index+1 << endl;
            v.resize(index+1);
        }
        return &v[index];
    }
};

// standard cartesian x,y N S W E
const vector<int> dx = {0,0,-1,1};
const vector<int> dy = {1,-1,0,0};

int clockwise(int dir){
    if(dir == 0) return 3;
    if(dir == 3) return 1;
    if(dir == 1) return 2;
    if(dir == 2) return 0;
}

int anticlockwise(int dir){
    if(dir == 3) return 0;
    if(dir == 1) return 3;
    if(dir == 2) return 1;
    if(dir == 0) return 2;
}

int main(){
    Computer pc(program);

    map<pair<int,int>, int> m;
    int minX = 0, minY=0,maxX=0,maxY=0;

    m[{0,0}] = 1;
    int x=0,y=0;
    int dir = 2; // 0 indexed
    while(true) {
        pc.in.push(dir+1);

        int steps = 0;
        while(pc.step()){
            steps++;
        }
        cout << "took " << steps << " steps" << endl;
        
        int out = pc.out.front(); pc.out.pop();
        cout << out << endl;

        if(out == 0){
            m[{x+dx[dir], y+dy[dir]}] = 0;
        }
        if(out == 1){
            x += dx[dir]; y += dy[dir];
            m[{x,y}] = 1;
        }
        if(out == 2){
            x += dx[dir]; y += dy[dir];
            m[{x,y}] = 2;
        }

        minX = min(minX, x); minY = min(minY, y);
        maxX = max(maxX, x); maxY = max(maxY, y);

        for(int y1=maxY+1; y1>=minY-1; y1--){
            for(int x1=minX-1; x1<=maxX+1; x1++){
                if(x == x1 && y == y1){
                    cout << "[]"; continue;
                }
                if(m.find({x1,y1}) == m.end()){
                    cout << "  "; continue;
                }
                int tile = m[{x1,y1}];
                if(tile == 0) cout << "##";
                else if(tile == 1) cout << "..";
                else if(tile == 2) cout << "OX";
            }
            cout << endl;
        }

        if(out == 1){
            dir = anticlockwise(dir);
        }
        if(out != 1){
            dir = clockwise(dir);
        }

        if(x == 0 && y == 0 && dir == 2)
            break;
    }

    int oxygenX, oxygenY;
    for(auto [pos, tile] : m){
        if(tile == 2){
            auto [ x, y ] = pos;
            oxygenX = x;
            oxygenY = y;
        }
    }

    // now do shortest path algo
    queue<tuple<int,int,int>> q;
    map<pair<int,int>, int> dist;
    map<pair<int,int>, bool> state;
    q.push({oxygenX,oxygenY,0});
    
    int furthest_space_dist = 0;
    while(!q.empty()){
        auto [x,y,d] = q.front(); q.pop();
        if(state[{x,y}] != 0) continue;
        if(x == 0 && y == 0)
            cout << "dist to oxygen = " << d << endl;
        dist[{x,y}] = d;
        state[{x,y}] = 1;

        if(d > furthest_space_dist)
            furthest_space_dist = d;

        for(int dir=0;dir<4;dir++){
            int x1 = x + dx[dir];
            int y1 = y + dy[dir];
            if(m[{x1,y1}] != 0)
                q.push({x1,y1,d+1});
        }
    }

    cout << "furthest dist:" << furthest_space_dist << endl;


    return 0;
}