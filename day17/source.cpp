...
int main(){
    ...
    vector<vector<char>> A(50, vector<char>(51));
    {
        int i=0,j=0;
        while(pc.out.size() != 0){
    ...
    cout << "intersection sum:" << sum << endl;

    // now lets trace the whole path (as though no memory limit)
    int i=startI;
    int j=startJ;
    int dir = 0; // North
    ...
    {        
        ...
        int i2 = i + di[(dir+3)%4];
        int j2 = j + dj[(dir+3)%4];
        if(i1 >= 0 && i1 <= 49 && j1 >=0 && j1 <= 50
            && A[i1][j1]=='#')
        {
            cout << "R";
            dir = (dir+1)%4;
        }
        else if(i2 >= 0 && i2 <= 49 && j2 >=0 && j2 <= 50
            && A[i2][j2]=='#')
        {
            cout << "L";
            dir = (dir+3)%4;
