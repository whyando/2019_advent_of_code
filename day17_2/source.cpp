...
void run_to_wait(Computer &pc){
    ...
    while(pc.out.size() != 0){
        int64_t out = pc.out.front(); pc.out.pop();
        if(out > 255)
            cout << out << endl;
        else
            cout << (char)out;
...
void send_ascii_input(Computer &pc, string s){
    for(char c : s){
        if(c == 0) continue; // ignore null termination char
        pc.in.push((int)c);
    }
}
...
int main(){
    ...
    run_to_wait(pc);
    send_ascii_input(pc, "A,B,A,B,C,B,A,C,B,C\n");
    run_to_wait(pc);
    send_ascii_input(pc, "L,12,L,8,R,10,R,10\n");
    run_to_wait(pc);
    send_ascii_input(pc, "L,6,L,4,L,12\n");
    run_to_wait(pc);
    send_ascii_input(pc, "R,10,L,8,L,4,R,10\n");
    run_to_wait(pc);
    send_ascii_input(pc, "n\n");
    run_to_wait(pc);
