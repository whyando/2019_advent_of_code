...
vector<tuple<int,int,int,int>> parseInput(vector<string> input){
    vector<tuple<int,int,int,int>> wireList;
...
int main(){
    ...
    for(auto [x1,y1,x2,y2] : wires1) {
        for(auto [x3,y3,x4,y4] : wires2) {
            if(!(x1 <= x2)) swap(x1, x2);
            if(!(x3 <= x4)) swap(x3, x4);

            // then xStart to xFinish is the overlapping interval
            int xStart = max(x1, x3);
            int xFinish = min(x2, x4);

            if(!(y1 <= y2)) swap(y1, y2);
            if(!(y3 <= y4)) swap(y3, y4);

            // then xStart to xFinish is the overlapping interval
            int yStart = max(y1, y3);
            int yFinish = min(y2, y4);
